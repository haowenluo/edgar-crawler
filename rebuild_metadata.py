...
Usage:
    # Scan and rebuild metadata for 10-K filings
    python rebuild_metadata.py --filing-type 10-K

    # Rebuild for all filing types
    python rebuild_metadata.py --all

    # Preview what would be added without writing
    python rebuild_metadata.py --filing-type 10-K --dry-run
"""
...
class MetadataRebuilder:
    ...
    def __init__(self):
        ...
        self.metadata_backup = "datasets/FILINGS_METADATA_BACKUP.csv"
    ...
    def extract_metadata_from_file(self, filepath):
        ...
        This includes filing date, period of report, etc.
        """
        try:
    ...
    def rebuild_metadata(self, filing_types, dry_run=False, extract_from_files=True):
        ...
        if dry_run:
            ...
            print(new_df.head(10)[['CIK', 'Type', 'year', 'filename']].to_string())
        else:
            ...
            discovered_file = "datasets/FILINGS_METADATA_DISCOVERED.csv"
            new_df.to_csv(discovered_file, index=False)
            print(f"   ✅ Saved newly discovered entries to: {discovered_file}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Rebuild metadata from files on disk",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Rebuild metadata for 10-K filings
    python rebuild_metadata.py --filing-type 10-K
    ...
    # Fast mode (don't parse HTML files, only use filenames)
    python rebuild_metadata.py --filing-type 10-K --fast
        """
    )
    ...
    parser.add_argument(
        '--filing-type',
        type=str,
        help='Filing type to rebuild (e.g., 10-K, 10-Q, 8-K)'
    )

    parser.add_argument(
        '--all',
        action='store_true',
        help='Rebuild metadata for all filing types'
    )
    ...
    parser.add_argument(
        '--fast',
        action='store_true',
        help='Fast mode: only extract from filenames, don\'t parse HTML files'
    )

    args = parser.parse_args()

    # Determine filing types to process
    if args.all:
        filing_types = ['10-K', '10-Q', '8-K']
    elif args.filing_type:
        filing_types = [args.filing_type]
    else:
        print("❌ Please specify --filing-type or --all")
        return

    # Create rebuilder
    rebuilder = MetadataRebuilder()

    # Rebuild metadata
    extract_from_files = not args.fast
